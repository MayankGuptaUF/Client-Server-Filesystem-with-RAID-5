...
class BlockLayer():
    ...
    def BLOCK_NUMBER_TO_DATA_BLOCK(self, block_number, offset, length):
        return ''.join(MemoryInterface.get_data_block(block_number, offset, length))
    ...
    def free_data_block(self, block_number, server):
        MemoryInterface.free_data_block(block_number, server)

    def free_data_block_id(self, block_number, server):
        MemoryInterface.free_data_block(block_number, server)
